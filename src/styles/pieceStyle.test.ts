...
describe('piece visual styles', () => {
  ...
  it('uses high-contrast solid piece text for both sides', () => {
    ...
    expect(pointRule).toContain('width: min(clamp(44px, 9.6vw, 76px), 11.8cqw)');
    expect(pointRule).toContain('height: min(clamp(44px, 9.6vw, 76px), 11.8cqw)');
    expect(pointRule).toContain('font-size: min(clamp(1.78rem, 4.2vw, 3rem), 7.4cqw)');
    ...
    expect(movingPieceRule).toContain('width: min(clamp(48px, 10vw, 80px), 12.2cqw)');
    expect(movingPieceRule).toContain('height: min(clamp(48px, 10vw, 80px), 12.2cqw)');
    expect(movingPieceRule).toContain('font-size: min(clamp(1.62rem, 4vw, 2.8rem), 7.2cqw)');
